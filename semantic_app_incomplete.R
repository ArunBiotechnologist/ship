...
ui <- semanticPage(
	...
	sidebarLayout(
        ...
        textOutput("currentTime", container = span)),
			br(),
			dropdown_input("type", default_text = h3("Select Vessel Type:"), 
   			 choices = unique(ship_table$ship_type), 
   			 value = unique(ship_table$ship_type)[1]),

			dropdown_input("name", default_text = h3("Choose Ship:"), 
   			 choices = "KAROLI"
					),
...
server <- function(input, output, session) {
	 ...
	 observeEvent(input$type, {
    			update_dropdown_input(session, input_id = "name", choices = ship_table %>% 
...
# Select the maximum traveelled locations
